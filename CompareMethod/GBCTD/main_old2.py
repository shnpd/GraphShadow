...
class CovertTransactionConstructor:
    ...
    def generate_topology(self, num_nodes, target_edge_count):
        ...
        print(
            f"[*] VGAE Topology Generated: {num_nodes} Nodes, {len(edges)} Edges (Units)"
        )
        return edges
    ...
    def algorithm_1_reconstruct_records(self, edge_units, node_labels, target_tx_count):
        ...
        for u_idx, v_idx in edge_units:
            ...
            S.append({"inputs": {u_addr}, "outputs": {v_addr}})  # 使用 set 方便集合运算

        current_count = len(S)
        print(
            f"[*] Start merging... Initial Units: {current_count}, Target: {target_tx_count}"
        )
        ...
        while len(S) > target_tx_count:
            if failures > max_failures:
                print(
                    f"[!] Warning: Max failures reached. Stopped at {len(S)} transactions."
                )
                break
    ...
    def construct_transactions(self, initial_utxos, total_input_slots_needed, pool_size=None):       
        """
        参数:
        initial_utxos: 初始可用的 UTXO 列表 (Source)
        total_input_slots_needed: 传输消息需要的总 Input 数量 (Edge Count)
        pool_size: (可选) 生成的中间/接收地址池大小 (Node Count)
                    如果不填，默认逻辑可以是 total_input_slots_needed 的一个比例，或者固定值。
        """
        num_inputs = len(initial_utxos)
        
        # ---------------------------------------------------------------------
        # [核心解耦逻辑] 1. 确定地址池大小 (Graph Nodes)
        # ---------------------------------------------------------------------
        if pool_size is None:
            # 默认策略：让图稍微稀疏一点，节点数约为边数的 0.8 倍 (平均度数 ~1.25)
            # 或者您可以设置为固定值，例如 max(200, total_input_slots_needed // 2)
            num_generated_addrs = max(int(total_input_slots_needed * 0.8), 50)
        else:
            # 用户指定了池子大小，完全解耦
            num_generated_addrs = pool_size

        print(f"[*] Configuration: Need {total_input_slots_needed} Inputs | Using {num_generated_addrs} Generated Addresses")
        ...
        print(
            f"[*] Final Edges Prepared: {len(final_edges)} (Ensured Initial UTXOs coverage)"
        )
        ...
        reconstructed_txs = self.algorithm_1_reconstruct_records(
            final_edges, node_labels, target_tx_count=target_R
        )
...
def main():
    ...
    initial_utxos = [f"1Addr_{i}" for i in range(1, 19)]
    ...
    for i in range (1,11):
        ...
        tx_list = constructor.construct_transactions(
            initial_utxos=initial_utxos, total_input_slots_needed=target_total_inputs
        )
