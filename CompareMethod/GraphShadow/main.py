...
if __name__ == "__main__":
    ...
    #         message_size_B=msg_size_B, 
    #         num_groups=18 # 使用之前的分组策略
    #     )
    #     # Save file
    #     output_filename = f"CompareMethod/GraphShadow/GraphShadow_transactions_{msg_size_B}.json"
    ...
    msg_size_B = 1024
    for i in range(1, 501):  # 1到100
        filename = f"CompareMethod/GraphShadow/dataset/GraphShadow_transactions_{i}.json"
        covert_tx, _ = generate_covert_transactions(
                message_size_B=msg_size_B, 
                num_groups=16 # 使用之前的分组策略
            )
        # Save file
